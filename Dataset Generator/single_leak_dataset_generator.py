...
results_folder = "../Dataset Generator/test/"
conf_path = "../Dataset Generator/single_leak_configuration.yalm"
...
class Leak_Generatior:
    def __init__(self):
        ...
        # self.leak_diameter_range = [0.01, 0.011, 0.012, 0.013, 0.014, 0.015, 0.016, 0.017, 0.018,
    ...
    def generate_leak(self, pipe_name, type):
        ...
        leak_pipe = wn.get_link(pipe_name)
        diameter = leak_pipe.diameter
        print(f"original pipe diameter: {diameter}")
        ...
        leakage_folder = f"{results_folder}{pipe_name}_{type}/"
...
if __name__ == "__main__":
    ...
    no_saved_pipe_list = [pipe for pipe in pipe_list if f'{pipe}_incipient' not in saved_pipe_list or f'{pipe}_abrupt' not in saved_pipe_list]
    ...
    slice_pipes = []
    # 划分管网为 10 份
    n = len(no_saved_pipe_list)
    step = (n + max_process - 1) // max_process  # 均分
    slice_pipes = [no_saved_pipe_list[i:i+step] for i in range(0, n, step)]
